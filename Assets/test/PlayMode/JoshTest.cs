...
public class JoshPlayModeTestsCooldown
{
   [UnitySetUp]
   public IEnumerator LoadScene()
   {
   ...
   {
      var cd = GameObject.FindFirstObjectByType<Cooldown>();
      Assert.NotNull(cd, "Cooldown not found in scene 'Demo'.");
      return cd;
   }

   [UnityTest] // Can set Cooldown Length
      public IEnumerator CooldownTests()
      {
         var cd = FindCooldown();
         cd.init(20);
         yield return null;
         Assert.AreEqual(20,cd.getCooldownLength(),"getCooldownLength() is messed up.");
         cd.setCooldownLength(30);
         Assert.AreEqual(30,cd.getCooldownLength(),"setCooldownLength() to a positive number is messed up.");
         cd.setCooldownLength(-1);
         Assert.AreEqual(30,cd.getCooldownLength(),"setCooldownLength() to a negative number is messed up.");
         cd.setCooldownLength(0);
         Assert.AreEqual(0,cd.getCooldownLength(),"setCooldownLength() to zero is messed up.");
      }
}
